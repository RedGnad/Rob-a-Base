 ...
 */
/*
  The sentry is the cone, and a modelled one was tried and taken down.

  A turret says "this shoots", and the sentry does not shoot, it seals, freezes and shakes coins
  loose: that is why it is an abstract cone and not a machine (owner, 4 Sep). An emitter was
  then built to stop the primitive reading as a placeholder, with a hexagonal pad, three leaning
  blades, a ring and a core, all inside the same envelope. The owner refused it on sight and the
  reason is the useful part: it was stretched boxes rather than a design, more complicated than
  the thing it decorated, and it evoked nothing (5 Sep). A prop that has to be explained has not
  earned its triangles.

  So the primitive comes back, and the lesson stays written: on a shape this small and this
  abstract, geometry does not buy character. If the cone is ever to become an object, it needs a
  drawn intent to follow, not more boxes.
*/
